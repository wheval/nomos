...
**Practical implication for Phase 5**: the shield-step worker cannot be a pure headless background job as originally scoped. Options to revisit when Phase 5 starts, in rough order of hackathon-friendliness: (a) a semi-manual flow where a team-controlled Ready/Xverse wallet performs the shield step (acceptable for hackathon-scale volume, not a real product answer), (b) investigate whether FPI offers any self-serve screening registration for a hackathon/testnet integration (worth asking in the Cairo CoreStars Telegram, per the docs' own suggestion for teams needing direct deposits), or (c) reconsider whether Flow B needs Nomos to self-shield at all versus some other settlement shape. Phase 6 (payout, via `withdraw`/`transfer`) is unaffected — screening is specific to deposits.
