...
export function runStoreContractTests(label: string, makeStore: () => Store) {
  describe(`Store contract (${label})`, () => {
    ...
    it("creates an open-amount payment link with no amountWei", async () => {
      const store = makeStore();
      const merchant = randomAddress();
      const link = await store.createPaymentLink({ merchantAddress: merchant, token: "STRK" });
      const fetched = await store.getPaymentLink(link.id);
      expect(fetched?.amountWei).toBeUndefined();
    });

    it("lists only the merchant's own payment links", async () => {
      const store = makeStore();
      const merchantA = randomAddress();
      const merchantB = randomAddress();
      await store.createPaymentLink({ merchantAddress: merchantA, token: "STRK" });
      await store.createPaymentLink({ merchantAddress: merchantA, token: "STRK" });
      await store.createPaymentLink({ merchantAddress: merchantB, token: "STRK" });
      expect(await store.listPaymentLinksFor(merchantA)).toHaveLength(2);
      expect(await store.listPaymentLinksFor(merchantB)).toHaveLength(1);
    });
